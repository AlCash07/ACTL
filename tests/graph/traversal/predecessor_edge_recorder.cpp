...
TEST_CASE("predecessor_edge_recorder bfs") {
    ...
    breadth_first_search{predecessor_edge_recorder{pred}, make_default_discovered_flag(graph)}(
        graph, 0);
    CHECK(std::vector<E>{E{}, graph.find_edge(0, 1), graph.find_edge(1, 2), graph.find_edge(0, 3),
                         graph.find_edge(3, 4), graph.find_edge(3, 5)} == pred);
}
...
TEST_CASE("predecessor_edge_recorder dfs") {
    ...
    using E = decltype(graph)::edge;
    std::vector<E> pred((size_t)graph.vertex_count());
    depth_first_search{predecessor_edge_recorder{pred}, make_default_discovered_flag(graph)}(graph,
                                                                                             0);
    CHECK(std::vector<E>{E{}, graph.find_edge(0, 1), graph.find_edge(1, 2), graph.find_edge(2, 3),
                         graph.find_edge(3, 4), graph.find_edge(4, 5)} == pred);
}
