...
TEST("bfs") {
    ...
    breadth_first_search{predecessor_edge_recorder{pred.data()},
                         make_default_discovered_flag(graph)}(graph, 0);
    ASSERT_EQUAL(
...
TEST("dfs") {
    ...
    using E = decltype(graph)::edge;
    std::vector<E> pred((size_t)graph.vertex_count());
    depth_first_search{predecessor_edge_recorder{pred.data()}, make_default_discovered_flag(graph)}(
        graph, 0);
    ASSERT_EQUAL(
