...
TEST_CASE("is_equal_sequence for vectors") {
    ...
    /* element mismatch */ {
        CHECK(!ac::is_equal_sequence(std::vector{4, 4}, std::vector{4, 2}));
        CHECK(!ac::is_equal_sequence(std::vector{5}, std::vector{4, 2}));
        CHECK(!ac::is_equal_sequence(std::vector{5, 2}, std::vector{4}));
    }
}
