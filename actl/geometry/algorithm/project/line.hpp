...
inline auto project(project_line_scalar<P, R>, const point<N, T0>& src, const line<N, T1, K>& dst) {
    return static_cast<geometry::ratio_t<R, T0, T1>>(dot<P>(src - dst.start, dst.slope)) /
           abs<P>(dst.slope);
}

template <class P, class R, int N, class T0, class T1, class K>
inline auto project(project_line<P, R>, const point<N, T0>& src, const line<N, T1, K>& dst) {
    return dst(project(project_line_scalar<P, R>(), src, dst));
}

template <int N, class T0, class T1, class K>
inline auto project(use_default, const point<N, T0>& src, const line<N, T1, K>& dst) {
