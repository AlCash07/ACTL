...
inline constexpr X area(comparable_area_points<P>, const point2d<T0>& lhs, const point2d<T1>& rhs) {
    return static_cast<X>(lhs[0]) * rhs[1] - static_cast<X>(lhs[1]) * rhs[0];
}

template <class P, class S, class T0, class T1>
inline constexpr auto area(standard_area_points<P, S>, const point2d<T0>& lhs,
...
inline auto area(comparable_area_points<P>, const point<N, T0>& lhs, const point<N, T1>& rhs) {
    return deferred_sqrt(abs<P>(lhs) * abs<P>(rhs) - sqr(dot<P>(lhs, rhs)));
}

template <class P, class S, int N, class T0, class T1>
inline auto area(standard_area_points<P, S>, const point<N, T0>& lhs, const point<N, T1>& rhs) {
    return static_cast<geometry::sqrt_t<S, T0, T1>>(area(comparable_area_points<P>(), lhs, rhs));
}

template <int N, class T0, class T1>
inline auto area(use_default, const point<N, T0>& lhs, const point<N, T1>& rhs) {
