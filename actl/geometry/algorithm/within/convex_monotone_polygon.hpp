...
inline int within(const within_convex_monotone_polygon<CP>& policy, const point2d<T0>& point,
    ...
    switch (ccw(policy, point, polygon[right], polygon[0])) {
        case 0: {
            ...
            return right == 1 || right + 1 == polygon.size() ? 0 : -1;
        }
        case -1: {  // lower chain
            auto lit = std::lower_bound(polygon.begin() + 1, polygon.begin() + right, point);
            return ccw(policy, lit[0], point, lit[-1]);
        }
        default: {  // upper chain
            auto uit = std::lower_bound(polygon.rbegin(), polygon.rend() - right - 1, point);
            return ccw(policy, uit == polygon.rbegin() ? polygon[0] : uit[-1], point, uit[0]);
        }
    }
}

template <class T0, class T1>
inline int within(use_default, const point2d<T0>& point,
                  const convex_monotone_polygon<T1>& polygon) {
