// Copyright 2021 Oleksandr Bacherikov.
//
// Distributed under the Boost Software License, Version 1.0.
// (See accompanying file LICENSE.txt or copy at http://www.boost.org/LICENSE_1_0.txt)

#pragma once

#include <actl/functional/scalar/comparison/cmp3way.hpp>
#include <actl/functional/scalar/comparison/equal.hpp>
#include <actl/functional/scalar/comparison/greater.hpp>
#include <actl/functional/scalar/comparison/greater_equal.hpp>
#include <actl/functional/scalar/comparison/less.hpp>
#include <actl/functional/scalar/comparison/less_equal.hpp>
#include <actl/functional/scalar/comparison/not_equal.hpp>
