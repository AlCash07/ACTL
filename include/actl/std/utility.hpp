...
inline bool equal(Policy&& policy, const std::pair<T1, T2>& lhs, const std::pair<T1, T2>& rhs) {
    return equal(policy, lhs.first, rhs.first) && equal(policy, lhs.second, rhs.second);
}

template <class Policy, class T1, class T2>
inline bool less(Policy&& policy, const std::pair<T1, T2>& lhs, const std::pair<T1, T2>& rhs) {
