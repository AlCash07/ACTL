...
template <class Device, class Format, class T, using C = char_t<Device>>
inline std::enable_if_t<std::is_trivially_copyable_v<T> && sizeof(T) % sizeof(C) == 0, index>
serialize(Device& od, Format&, const T& x, binary) {
    return od.write({reinterpret_cast<const C*>(&x), sizeof(T) / sizeof(C)});
}

template <class Device, class Format, class T, using C = char_t<Device>>
inline std::enable_if_t<std::is_trivially_copyable_v<T> && sizeof(T) % sizeof(C) == 0, bool>
