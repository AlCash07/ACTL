...
template <class Op, class Policy, class T, class U,
          enable_int_if<std::is_same_v<Op, Equal> || std::is_same_v<Op, Less>> = 0,
          enable_int_if<is_mimic_pair<T>::value || is_mimic_pair<U>::value> = 0>
inline auto perform(Op op, const Policy& policy, const T& lhs, const U& rhs) {
    return perform(op, policy, get_key(lhs), get_key(rhs));
}
