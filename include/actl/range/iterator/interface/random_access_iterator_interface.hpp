...
public:
    ...
    constexpr auto operator[](Difference offset) const
        AC_DEDUCE_NOEXCEPT_DECLTYPE_AND_RETURN(*(this->derived() + offset))
    ...
    friend constexpr auto operator+(DerivedIterator iter, Difference offset)
        AC_DEDUCE_NOEXCEPT_DECLTYPE_AND_RETURN(DerivedIterator{iter += offset})
    ...
    friend constexpr auto operator+(Difference offset, DerivedIterator iter)
        AC_DEDUCE_NOEXCEPT_DECLTYPE_AND_RETURN(iter + offset)
    ...
    ) const AC_DEDUCE_NOEXCEPT_AND_RETURN(*this <=> that == 0)
